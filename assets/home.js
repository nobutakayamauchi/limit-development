...
const FALLBACK_ITEMS=[
 {id:'invoice-payment-ops',name:'INVOICE PAYMENT OPS',short:'¥',kind:'PRODUCT / PRODUCTION',cats:['production','product'],killer:'請求した後まで、終わらせます。',desc:'見積・納品・請求・入金確認・領収書までを、同じ取引IDでChatGPTから扱います。',definition:'請求業務を最後まで閉じるための請求・入金管理セットです。',url:`${SALES_CATALOG}products/invoice-payment-ops/`,salesUrl:`${SALES_CATALOG}products/invoice-payment-ops/sales.html`},
 {id:'webai-bridge',name:'WEB AI BRIDGE',short:'W',kind:'PRODUCT / PRODUCTION',cats:['production','product'],killer:'AIを、自分の場所で使えるようにします。',desc:'AI機能を、自分のサイトや仕組みへつなぎやすくするための橋を作ります。',definition:'AIとWebをつなぐための接続基盤です。',url:'products/webai-bridge.html',salesUrl:`${SALES_CATALOG}products/webai-bridge/`},
 {id:'bridgepatch',name:'BRIDGE PATCH',short:'B',kind:'SERVICE / PRODUCTION',cats:['production','product'],killer:'ここ直せば、大体直ります。',desc:'問題の中心になっている一部分を見つけて、そこだけ小さく直したり、つなぎ直したりします。',definition:'全部作り直さず、効く場所へ手を入れる一点改善サービスです。',url:'products/bridgepatch.html',salesUrl:`${SALES_CATALOG}products/bridgepatch/sales.html`},
 {id:'axis',name:'AXIS',short:'X',kind:'SERVICE / RESEARCH',cats:['research','product'],killer:'たくさんあっても、今やるのはこれだけです。',desc:'仕事や課題が増えても、優先順位を整理して「今やること」をひとつに絞ります。',definition:'困っている一点を実装可能な仕様書と見積書へ落とす設計サービスです。',url:'products/axis.html',salesUrl:`${SALES_CATALOG}products/axis/sales.html`},
 {id:'ultimate-loop',name:'ULTIMATE LOOP',short:'∞',kind:'METHOD / RESEARCH',cats:['research'],killer:'一からの新造、やめてください。',desc:'今ある道具や過去の部品を先に探し、組み合わせても足りない部分だけを新しく作ります。',definition:'無駄な開発を減らすための開発手法です。',url:'products/ultimate-loop.html'},
 {id:'nagi',name:'NAGI',short:'N',kind:'PRODUCTION',cats:['production'],killer:'中断した場所？ここからです。',desc:'どこまで進んだか、次に何をするかを残して、途中から再開しやすくします。',definition:'中断と再開を楽にするツールです。',url:'products/nagi.html'},
 {id:'trace',name:'TRACE',short:'T',kind:'RESEARCH / EVIDENCE',cats:['research'],killer:'「何が起きた？」慌てずこれ見てください。',desc:'変更・失敗・修正の流れを記録して、あとから何が起きたかを追えるようにします。',definition:'トラブル時に状況をたどるための記録ツールです。',url:'products/trace.html'}
];
...
function normalizeCatalogItem(x){return {id:String(x.id||x.name||''),name:String(x.name||'UNTITLED'),short:String(x.short||'•'),kind:String(x.kind||'PUBLIC'),cats:Array.isArray(x.cats)&&x.cats.length?x.cats:['production'],killer:String(x.killer||x.summary||'GitHubで公開中です。'),desc:String(x.desc||x.summary||''),definition:String(x.definition||x.summary||''),url:String(x.url||x.repo_url||'#'),salesUrl:x.salesUrl?String(x.salesUrl):''}}
async function loadPublicCatalog(){
 try{const catalog=await loadJson('data/catalog.json');if(Array.isArray(catalog)&&catalog.length){ITEMS=catalog.filter(x=>x&&x.status!=='hidden').map(normalizeCatalogItem);visible=[...ITEMS.keys()];rebuild();return}}catch(_){/* generated catalog unavailable: keep reviewed fallback */}
 rebuild();
}
...
ensureSalesLinks();wireJournalNavigation();loadPublicCatalog();loadLiveBoards();
